...
# DEVELOPMENT NOTES on modules versions:
#   - Script modified to accept a --input_fastq with three columns
#   - Script edited to deal with single-end fastq files
...
def extract_type_of_fastq(fastq_dir):
    """ Returns "paired" or "single" depending on ending of fastq.gz files found in fastq_dir """
    type = None
    files = os.listdir(fastq_dir)
    fastq_files = [file for file in files if file.endswith(".fastq.gz")]
    fastq_paired_files = [file for file in fastq_files if file.endswith("_1.fastq.gz") or file.endswith("_2.fastq.gz")]
    if len(fastq_paired_files) == 2:
        type = "paired"
    if len(fastq_paired_files) == 0 and len(fastq_files) == 1:
        type = "single"
    return type
...
def _main():
    ...
    isolate_lane_dir = dict()  # dictionary to save isolate and lane ids
    saved = 0
    if args.input_fastq is not None:
        ...
        with open(args.input_fastq, 'r') as input_fastq_file:
            ...
            for input_fastq_line in input_fastq_file:
                    ...
                    (isolate_id, lane_id, fastq_dir) = input_fastq_line.strip().split('\t')
                    isolate_fastq_dir[isolate_id] = fastq_dir
                    isolate_lane_dir[isolate_id] = lane_id
                    saved = saved + 1
                    print('isolate_fastq_dir[' + isolate_id + '] ' + isolate_fastq_dir[isolate_id])
    print('Total number of fastq dir saved in isolate_fastq_dir: ' + str(saved) + '\n')
            ...
            for output_line in output_file:
                ...
                for isolate_to_be_mapped in isolates_to_be_mapped:
                    ...
                    if isolate_fastq_dir.get(isolate_to_be_mapped) is None:
                        ...
                        sys.exit(-1)
                    if isolate_lane_dir.get(isolate_to_be_mapped) is None:
                        logging.error(f'Lane id of isolate {isolate_to_be_mapped} not found!')
                        sys.exit(-1)
                    type = extract_type_of_fastq(isolate_fastq_dir[isolate_to_be_mapped])
                    fastq_file = isolate_fastq_dir[isolate_to_be_mapped] + '/' + isolate_lane_dir[isolate_to_be_mapped] + '.fastq.gz'
                    fastq1_file = isolate_fastq_dir[isolate_to_be_mapped] + '/' + isolate_lane_dir[isolate_to_be_mapped] + '_1.fastq.gz'
                    fastq2_file = isolate_fastq_dir[isolate_to_be_mapped] + '/' + isolate_lane_dir[isolate_to_be_mapped] + '_2.fastq.gz'
                    # Making sure fastq.gz files exist
                    if type is None:
                        logging.error(f'Type of fastq.gz files in {isolate_fastq_dir[isolate_to_be_mapped]} could not be determined!')
                        sys.exit(-1)
                    if type == "single":
                        if not os.path.exists(fastq_file):
                            logging.error(f'Input file {fastq_file} not found!')
                            sys.exit(-1)
                    if type == "paired":
                        if not os.path.exists(fastq1_file):
                            logging.error(f'Input file {fastq1_file} not found!')
                            sys.exit(-1)
                        if not os.path.exists(fastq2_file):
                            logging.error(f'Input file {fastq2_file} not found!')
                            sys.exit(-1)
                    output_snippy_vcf_file = args.vcf_dir + pair_id + '.snippy.vcf'
                    # Creating snippy farm jobs
                    bsub_line = ""
                    if type == "paired":
                        bsub_line = 'bsub -q normal -G team346 -J ' + pair_id + ' -o ' + pair_id + '.out -n8 ' + \
                                    '-R\"span[hosts=1] select[mem > 50000] rusage[mem=50000]\" -M 50000 ' + \
                                    '\"bash run_snippy_per_sample.farm.sh ' + snippy_out_dir + ' ' + reference_assembly + \
                                    ' ' + fastq1_file + ' ' + fastq2_file + ' ' + output_snippy_vcf_file + '\"'
                    if type == "single":
                        bsub_line = 'bsub -q normal -G team346 -J ' + pair_id + ' -o ' + pair_id + '.out -n8 ' + \
                                    '-R\"span[hosts=1] select[mem > 50000] rusage[mem=50000]\" -M 50000 ' + \
                                    '\"bash run_snippy_per_sample.single.farm.sh ' + snippy_out_dir + ' ' + reference_assembly + \
                                    ' ' + fastq_file + ' ' + output_snippy_vcf_file + '\"'
                    print(bsub_line)
